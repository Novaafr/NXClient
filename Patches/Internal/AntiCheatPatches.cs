...
namespace NXClient.Patches.Internal
{
    public class AntiCheatPatches
    {
        ...
        public class SendReportPatch
        {
            ...
            {
                if (Main.showanticheatreports)
                {
                    Notifications.NotifiLib.SendNotification($"<color=yellow>[ANTICHEAT]</color> Reported {susNick} for {susReason}");
                }
                return false;
            }
        }
