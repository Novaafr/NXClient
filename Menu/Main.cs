...
using static NXClient.Menu.Buttons;
    ...
    public class Main : MonoBehaviour
    {
        ...
        public static void Prefix()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (rightHanded)
                            comp.linearVelocity = GTPlayer.Instance.rightHand.velocityTracker.GetAverageVelocity();
                        else
                            comp.linearVelocity = GTPlayer.Instance.leftHand.velocityTracker.GetAverageVelocity();
            ...
            try
            {
                if (triggerpages)
                {
                    if (ControllerInputPoller.instance.rightControllerIndexFloat > 0.6f && !cooldown)
                    {
                        cooldown = true;
                        Toggle("NextPage");
                    }
                    if (ControllerInputPoller.instance.leftControllerIndexFloat > 0.6f && !cooldown)
                    {
                        cooldown = true;
                        Toggle("PreviousPage");
                    }
                    if (ControllerInputPoller.instance.rightControllerIndexFloat == 0f && ControllerInputPoller.instance.leftControllerIndexFloat == 0f && cooldown)
                        cooldown = false;
                }
            }
            catch { }
        ...
        public static void CreateMenu()
        {
            ...
            if (outline)
            {
                GameObject outlinee = GameObject.CreatePrimitive(PrimitiveType.Cube);
                Destroy(outlinee.GetComponent<Rigidbody>());
                Destroy(outlinee.GetComponent<BoxCollider>());
                outlinee.transform.parent = menu.transform;
                outlinee.transform.rotation = Quaternion.identity;
                outlinee.transform.localScale = menuSize * 0.2f;
                outlinee.GetComponent<Renderer>().material.color = new Color32(225, 216, 223, 255);
            }
            ...
            gameObject.transform.localPosition = triggerpages ? Vector3.zero : new Vector3(0.56f, 0.65f, 0);
                ...
                else
                {
                    ...
                    if (target != null)
                    {
                        if (target.isTogglable)
                        {
                            ...
                            if (target.enabled)
                            {
                                NotifiLib.SendNotification("<color=green>[ENABLE]</color> " + target.toolTip);
                                if (target.enableMethod != null)
                                    try { target.enableMethod.Invoke(); } catch { }
                            }
                            else
                            {
                                NotifiLib.SendNotification("<color=red>[DISABLE]</color> " + target.toolTip);
        ...
        {
            Transform gun = GorillaTagger.Instance.rightHandTransform;

            Vector3 startPos = gun.position;
            Vector3 dir = gun.forward;

            Physics.Raycast(startPos, dir, out var hit, 512f, overrideLayerMask ?? NoInvisLayerMask());

            Vector3 endPos = gunLocked && lockTarget != null
                ? lockTarget.transform.position
                : hit.point;
            ...
            if (GunPointer == null)
            {
                ...
                GunPointer.transform.localScale = Vector3.one * 0.2f;
                if (GunPointer.TryGetComponent<Collider>(out var col))
